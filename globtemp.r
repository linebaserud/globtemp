...
period <- 'Jan'    # type of data, e.g. 'Yearly', 'Jan', or 'Feb', ...
...
if (!is.na(match('NASA',datasets)) & length(datasets)>1){
  ...
  p=p+annotate("text",x=y1+(2020-y1)/2-25,y=0.95,label="NASA/GISS",color="orange",size=6)  
}
...
if (!is.na(match('HadCRUT',datasets)) & length(datasets)>1){
  ...
  p=p+annotate("text",x=y1+(2020-y1)/2+15,y=0.95,label="HadCRUT4",color="brown",size=6)  
}
