...
globtemp <- function(datasets, refs, refe, period, save_option, save_name){
  ...
  plot_single_bars <- function(p, df_val, df_y, df_col, new_mean, df_name) {
    p <- p +
         labs(subtitle = paste(df_name)) +
  ...
  plot_plural <- function(p, df_val, df_y, new_mean, line_color, df_name, last_data_point, text_pos_offset) {
    p <- p +
         ...
         annotate("text", x = y1 + (year_max - y1)/2 - text_pos_offset, y = 0.95, label = df_name, color = line_color, size = 6) +
         annotate("text", x = y1 + (year_max - y1)/2 - text_pos_offset, y = 0.85, label = paste0("(", last_data_point, ")"), color = line_color, size = 4)
    return(p)
  }
  ...
  if (!is.na(match('NASA', datasets)) & length(datasets) == 1) {
    p <- plot_single_bars(p, D$val, D$y, D$col, m_new, textNASA) 
  ...
  if (!is.na(match('Copernicus', datasets)) & length(datasets) == 1){
    ...
    p <- plot_single_bars(p, D2$val, D2$y, D2$col, m2_new, textCopernicus) 
  ...
  if (!is.na(match('HadCRUT', datasets)) & length(datasets) == 1){
   # p <- plot_single_points(p, D3$val, D3$y, D3$col, m3_new, textHadCRUT) 
    p <- plot_single_bars(p, D3$val, D3$y, D3$col, m3_new, textHadCRUT) 
